            ...
            <div className="ct-card p-4 mb-6">
              ...
              <div aria-hidden="true">
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={chartData}>
                    ...
                    <XAxis dataKey="week" tick={{ fontSize: 11, fill: 'var(--ink-muted)' }} axisLine={{ stroke: 'var(--border)' }} />
                    <YAxis tick={{ fontSize: 11, fill: 'var(--ink-muted)' }} axisLine={{ stroke: 'var(--border)' }} />
                    <Tooltip
                      contentStyle={{
              ...
              <table className="sr-only">
                ...
                <tbody>
                  {weeks.map((w) => (
                    <tr key={w.week_start}>
                      <td>{new Date(w.week_start).toLocaleDateString('en-IN', { month: 'long', day: 'numeric' })}</td>
              ...
              {weeks.map((w) => (
                <div key={w.week_start} className="text-center">
                  ...
                  <p className="text-[10px] font-mono-num mt-1" style={{ color: 'var(--ink-muted)' }}>
                    {new Date(w.week_start).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}
                  </p>
                </div>
              ))}
            </div>
