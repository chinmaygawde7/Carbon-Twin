...
export default function Home() {
  ...
  const [pendingConfirm, setPendingConfirm] = useState<{ rawInput: string; category: string | null } | null>(null)
  ...
  const primary = allActions.filter((a) => PRIMARY_ACTIONS.includes(a.key))
  const rest = allActions.filter((a) => !PRIMARY_ACTIONS.includes(a.key))
  ...
  async function handleConfirm(category: string) {
    ...
    const result = await logConfirmedAction({ source: 'voice', category, rawInput: pendingConfirm.rawInput })
  ...
  if (!authChecked) {
    ...
    return (
      <main className="px-5 py-8 max-w-md mx-auto w-full">
        ...
        {suggestion && (
          <Suggestion recommendation={suggestion} onLog={handleLog} />
        )}
        ...
        <div className="mt-6">
          {pendingConfirm ? (
            ...
            <ReceiptConfirm items={pendingReceipt} onConfirm={handleReceiptConfirm} onCancel={() => setPendingReceipt(null)} />
          ) : (
            <div className="flex gap-6 justify-center items-start">
              <button onClick={() => (document.querySelector('[data-voice-trigger]') as HTMLElement)?.click()} className="hidden" />
              <div className="flex flex-col items-center gap-2">
                <VoiceLog onParsed={(transcript, result) => setPendingConfirm({ rawInput: transcript, category: result.category })} />
              </div>
              <div className="flex flex-col items-center gap-2">
